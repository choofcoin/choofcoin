...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("choofcoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #choofcoinTEST2\r");
            Send(hSocket, "WHO #choofcoinTEST2\r");
        } else {
            // randomly join #choofcoin00-#choofcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #choofcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #choofcoin%02d\r", channel_number).c_str());
        }
